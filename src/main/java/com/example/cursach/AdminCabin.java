...
public class AdminCabin {
    ...
    void initialize() throws SQLException, ClassNotFoundException {
        ...
        roles_menu.setItems(FXCollections.observableArrayList(db.getNameRoles()));
    }
    ...
    public void addUser() throws SQLException, ClassNotFoundException {
        ...
        if (login_input.getText().length() == 0 || password_input.getText().length() == 0 ||
            ...
            windowMessenger("Присутствуют незаполненные обязательные поля!");
        } else {
            // Проверка на уникальность логина
            if (db.isDuplicateLogin(login_input.getText())) {
                windowMessenger("Такой логин уже используется другим пользователем!");
            } else {
                ...
                if (Objects.equals(roles_menu.getValue(), "Спортсмен")) {
                    ...
                    if (birth_country_input.getText().length() == 0 || birth_date_input.getValue() == null) {
                        windowMessenger("Пресутсвуют незаполненные обязательные поля для спортсмена!");
                    } else {
                        ...
                        if (Period.between(birth_date_input.getValue(), LocalDate.now()).getYears() < 7) {
                            windowMessenger("Спортсмен/ка слишком мал/а");
                        } else {
                            // Добавляем нового спортсмена
                            db.addNewAthletes(roles_menu.getValue(), login_input.getText(), password_input.getText(),
                    ...
                    db.addNewUser(roles_menu.getValue(), login_input.getText(), password_input.getText(),
                            fio_input.getText());
                    clearCells();
                }
            }
    ...
    void loadGradingCriteria() throws IOException {
         ...
         */
        FXMLLoader fxmlLoader = new FXMLLoader(HelloApplication.class.getResource("admin-add-gradingcriteria.fxml"));
    ...
    void editUser() throws SQLException, ClassNotFoundException {
        ...
        if (login_input.getText().length() == 0 || password_input.getText().length() == 0 ||
            ...
            int idRole = db.getIdRole(login_input.getText(), password_input.getText());
            // Проверка на изменения у администратора
            if (idRole != 5) {
                if (Objects.equals(roles_menu.getValue(), "Спортсмен")) {
                    // Если у спортсмена дата и родина не заполнены, меняем только ФИО
                    if (birth_country_input.getText().length() == 0 || birth_date_input.getValue() == null) {
                        db.editUser(idRole, login_input.getText(), password_input.getText(), fio_input.getText());
                    } else {
                        ...
                        if (Period.between(birth_date_input.getValue(), LocalDate.now()).getYears() < 7) {
                            ...
                            db.editUser(login_input.getText(), password_input.getText(), fio_input.getText(),
                ...
                windowMessenger("У администратора нельзя менять данные!");
            }
        }
    ...
    void deleteUser() throws SQLException, ClassNotFoundException {
        ...
        // Проверка на обязательные поля
        if (login_input.getText().length() != 0 || password_input.getText().length() != 0) {
            db.deleteUser(login_input.getText(), password_input.getText());
            windowMessenger("Администратора нельзя удалять");
        }
        clearCells();
    }
